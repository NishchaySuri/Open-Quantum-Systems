...
class SetUpKrotov:
    ...
    def __init__(self,x_i,x_f,dx,t_i,t_f,dt,alpha,delta,eta):
        ...
        self.k_f = + np.pi/self.dx
        self.k = self.k_i + self.dk * np.arange(self.num_x)
    ...
    def mu(self,ket):
        ...
        for i in range(0,len(ket)):
            ...
            val = mu0 * i * np.exp(-self.x[i]/xStar)
    ...
    #Compute the Commutator [mu,H]|ket>
    ...
    def Update_State(self,string):
        ...
        if string == 'Chi':
            ...
            self._H = self._Ho + self._V - (self.E[self.t_index] * self._mu)
            # Call diaglonalize here ! if not diagonalized yet !
            
            ket = self.Fourier(ket)
            ket = np.exp( 1.j * self.k**2/(2*m) * self.dt/2) * ket
            ket = self.InvFourier(ket)
...
if __name__ == "__main__" :
    k=SetUpKrotov(-10,10,0.1,0,1000,0.1,1,1,1)
    k.Start_Krotov(10)
    plt.plot(k.overlap)
    plt.show()

...
class SetUpEquation:
    ...
    # Krotov Parameters. #Use delta = 2 , eta = 0 
        self.delta = 1
        self.eta = 1
        self.alpha = .01
     ...
     # Columnized Density Matrices.
        ...
        self.rho_thermal = np.array([[self.n_bar/(2*self.n_bar+1),0],[0,(self.n_bar+1)/(2*self.n_bar+1)]])
#        self.rho_thermal = 0.5 * np.array([[1,1],[1,1]])

     # Defining Ex(t) and Ez(t).
        self.Ex = np.zeros((self.num_t,))
        self.Ex_prev = np.zeros((self.num_t,))
        self.Ex_tilda = np.zeros((self.num_t,))
        self.Ex_tilda_prev = np.zeros((self.num_t,))
...
#        self.Psi[0][:] = self.Matrix_2_Column(self.rho_Init())
        self.Psi[0][:] = 0.5 * np.array([1,1.j,-1.j,1],dtype='complex')
   ...
   # Takes a column Density matrix and returns square Density Matrix
    ...
    # Retuerns Overlap of| self.rho_thermal | Psi(t)>
    def O(self,ket):
        
        col_den_matrix = self.Matrix_2_Column(self.rho_thermal)
    ...
    # Retuerns Overlap of <Psi(t) | self.self.rho_thermal | Psi(t)>
    def Overlap(self,t):
        ket = self.Psi[t]
...
#        _commutator = np.dot(self._Col_muX,_H) - np.dot(_H,self._Col_muX)
        ...
        # Define operator = rho mu - mu rho
        _op = self.rho_action_Op(self._muX) - self.Op_action_rho(self._muX)
        
        ket_new = np.dot(1.j*_op,ket) # - (self.dt) * np.dot(_commutator,ket)
...
#        _commutator = np.dot(self._Col_muX,_H) - np.dot(_H,self._Col_muX)
    ...
    def distance(self,t):
        ...
        X = self.rho_thermal - matrix

        trace = np.trace(scipy.linalg.sqrtm(np.dot(X,np.conjugate(np.transpose(X)))))
...
if __name__ == '__main__':
    #(N,k,n_bar,omega,Jx,Jy,t_i,t_f,num_t):
    
    s = SetUpEquation(1,.01,1,10,0,0,0,10,5000)
    ...
    # Start Krotov
    k = SetUpEquation(1,.01,1,10,0,0,0,10,5000)
    ...
    for i in range(0,len(s.t)):
        heat1.append(np.real(s.Heat(i)))
        heat2.append(np.real(k.Heat(i)))
    ...
    plt.xlabel('Time')
    plt.plot(s.t,heat1)
    plt.plot(k.t,heat2,'r')

    plt.figure(3)
    plt.title('Epsilon Vs Time')
    plt.ylabel('Epsilon')
    plt.xlabel('Time')
    plt.plot(k.t,np.real(k.Ex))

    work1 = []
    work2 = []
    
    for i in range(0,len(s.t)-1):
        work1.append(np.real(s.Work(i)))
        work2.append(np.real(k.Work(i)))
        
    plt.figure(4)
    plt.title('Work Vs Time')
    plt.ylabel('Work')
    plt.xlabel('Time')
    plt.plot(s.t[:-1],work1)
    plt.plot(k.t[:-1],work2,'r')

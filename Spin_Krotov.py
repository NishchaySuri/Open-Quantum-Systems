...
class SetUpEquation:
   ...
   # Takes a column Density matrix and returns square Density Matrix
    ...
    def Overlap(self):
        return(self.Psi[s.num_t-1][self.num_basis**2-1]**2)
    ...
    def Update_Ex_tilda_Ez_tilda(self):
        ...
        # Update Ex_tilda
        _H = self._H0 + self._Col_muZ * self.Ez[t]

        # Commutator [muX,H0 + muZ*Ez(t)]
        _commutator = self.Op_action_rho(np.dot(self._Col_muX,_H)) - self.Op_action_rho(np.dot(_H,self._Col_muX))
        ...
        # Update Ez_tilda
        _H = self._H0 + self._Col_muX * self.Ex[t]
        ...
        _Zpart1 = (1 - self.eta) * self.Ez[self.t_index]


        self.Ex_tilda[self.t_index] = _Xpart1 + _Xpart2
        self.Ez_tilda[self.t_index] = _Zpart1 + _Zpart2
    ...
    def Update_Ex_Ez(self):
        ...
        _Zpart2 = np.real(np.vdot(bra, ket_new)) * (self.delta / self.alpha)

        _Zpart1 = (1 - self.delta) * self.Ez_tilda[self.t_index]


        self.Ex[self.t_index] = _Xpart1 + _Xpart2
        self.Ez[self.t_index] = _Zpart1 + _Zpart2
    ...
    def Update_State(self,string):
        ...
        if string == 'Psi':
           ...
           # Diagonalize the Matrix.
            H_dagger = - np.conjugate(np.transpose(self.H()))
...
if __name__ == '__main__':
    s = SetUpEquation(2,1,1,1,0,0,0,1,1000)
